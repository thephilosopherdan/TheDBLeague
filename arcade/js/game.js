...
const ER_TEAMS = [
    { team: 'Windy City Wankers',       score: 73.00,  ownerId: '818631532867641344' },
    { team: 'Papa Bear Down Chi-Town',  score: 105.00, ownerId: '1125627970648895488' },
    { team: 'The Peeps of America',     score: 125.83, ownerId: '1294496852862865408' },
    { team: 'North End Cannolis',       score: 143.00, ownerId: '399425495596290048' },
    { team: 'Brotherly Love Broskis',   score: 147.00, ownerId: '858901414712287232' },
    { team: 'Vermont Maple Baddies',    score: 163.00, ownerId: '856696567774683136' },
    { team: 'Belgian Tripel Mama',      score: 185.00, ownerId: '814297857317240832' },
    { team: 'Warren-G Regulators',      score: 232.00, ownerId: '818613649684946944' },
    { team: 'Reno Deputies',            score: 233.00, ownerId: '815024153592745984' },
    { team: 'South Shore Dirty Dawgs',  score: 294.00, ownerId: '368921092460462080' }
];
...
const ARCADE = (() => {
    ...
    const TAUNTS = ['The goal line does not yield.', 'Nobody beats the Dirty Dawgs.',
                    'Commissioner’s forecast: 100% chance of defeat.', 'You cannot outrun the pound.'];
    const RUNNERS = {
    ...
    let SAVE;
    const persist = () => { try { localStorage.setItem('endzone_run_db_v1', JSON.stringify(SAVE)); } catch (e) {} };
    ...
    const MASCOT2F = [];
    const MASCOT1F = ['dawgs', 'peeps', 'mama', 'broskis', 'cannolis', 'papabear',
                      'reno', 'maple', 'regulators', 'wankers'];
    ...
    const TEAM_MASCOT = {
        '368921092460462080':  'dawgs',      '815024153592745984': 'reno',
        '818613649684946944':  'regulators', '814297857317240832': 'mama',
        '856696567774683136':  'maple',      '858901414712287232': 'broskis',
        '399425495596290048':  'cannolis',   '1294496852862865408': 'peeps',
        '1125627970648895488': 'papabear',   '818631532867641344': 'wankers'
    };
            ...
            if (st.lockd) {
                overlayHTML(`<div style="text-align:center">
                    <div style="display:inline-block;background:linear-gradient(160deg,#06301C,#0B5230);border-left:8px solid #E8C547;
                        ...
                        <p class="display" style="color:#E8C547;font-size:13px;letter-spacing:0.2em;margin:0">🏆 2025 DB TROPHY CHAMPIONS 🏆</p>
                    </div>
                    <div><img src="assets/champ_logo.png"
                    ...
                    <h2 class="display" style="color:#E8C547;font-size:22px;text-transform:uppercase;margin:12px 0 6px">Nobody Beats the<br>South Shore Dirty Dawgs</h2>
                    <p class="ag-sub" style="text-align:center;max-width:380px;margin:0 auto 6px">Ten downs of your absolute best stuff… and the junkyard
                       dawgs never even blinked. Console yourself with this: The Briggs is still up for grabs, and the
                       trophy was staying at the bar either way.</p>
    ...
    function draw() {
        ...
        Composite.allBodies(engine.world).forEach(b => {
            ...
            if (b.gtype === 'sled') {
                ...
                const mk = levelMascot();
                const im = (mk && IMG['m_' + mk]) || (b.elite ? IMG.def_elite : IMG.def_stand);
    ...
    function vsScreen() {
        const team = TEAMS[level];
        const mk = levelMascot();
        const mfile = mk ? 'assets/m_' + mk + '.png' : 'assets/def_stand.png';
        const mimg = mk ? IMG['m_' + mk] : IMG.def_stand;
    ...
    function mount(container) {
        try { SAVE = Object.assign({}, DEFSAVE, JSON.parse(localStorage.getItem('endzone_run_db_v1') || '{}')); }
    ...
    return {
        ...
        reset: () => { if (confirm('Wipe all arcade progress?')) { localStorage.removeItem('endzone_run_db_v1'); SAVE = { ...DEFSAVE }; calibrate(); titleScreen(); } }
    };
})();
