// ═════════════════════════════════════════════════════════════════════════
//  THE D.B. LEAGUE — APPLICATION
//  Sections: HQ · League · Recap · Dynasty · The Wire · Analytics · Trade Lab · Legacy
// ═════════════════════════════════════════════════════════════════════════

const APP = { section: 'hq', wireTab: 'activity', legacyTab: 'records',
              labA: null, labB: null, labSelA: new Set(), labSelB: new Set(),
              recapTab: null, recapWeek: null, recapSeason: null, dynastyTab: 'official',
              sheetLive: false };
...
const avatarUrl = av => {
    const s = String(av);
    return s.startsWith('http') || s.startsWith('logos/') ? s : `https://sleepercdn.com/avatars/thumbs/${s}`;
};
const avatarImg = (av, cls) => av
    ? `<img src="${avatarUrl(av)}" class="${cls} object-cover" loading="lazy" onerror="this.outerHTML='<div class=\'${cls} flex items-center justify-center bg-white/5\'><i class=\'fa-solid fa-beer-mug-empty text-slate-600\'></i></div>'">`
    : `<div class="${cls} flex items-center justify-center bg-white/5"><i class="fa-solid fa-beer-mug-empty text-slate-600"></i></div>`;

function spinner(msg) {
    return `<div class="flex flex-col items-center py-24">
        <div class="w-10 h-10 border-2 border-white/10 border-t-[#F5A623] rounded-full animate-spin mb-4"></div>
...
function toast(msg) {
    ...
    t.textContent = msg;
    t.style.cssText = 'position:fixed;bottom:90px;left:50%;transform:translateX(-50%);z-index:200;background:#161310;border:1px solid rgba(245,166,35,0.4);color:#F5A623;padding:10px 18px;border-radius:999px;font-family:JetBrains Mono,monospace;font-size:11px;letter-spacing:0.08em;box-shadow:0 8px 24px rgba(0,0,0,0.5);opacity:1;transition:opacity .4s';
    clearTimeout(t._h); t._h = setTimeout(() => t.style.opacity = '0', 2600);
}
function sectionHead(kicker, title, sub) {
    return `<header class="mb-8">
        <p class="mono text-[10px] uppercase tracking-[0.35em] text-[#F5A623]/70 mb-2">${esc(kicker)}</p>
...
function sparkline(series, w = 120, h = 30, color = '#F5A623') {
...
const currentBriggs = () => BRIGGS_WINNERS[0];

function badgesHTML(dyn, isChamp) {
    let h = '<div class="flex flex-wrap gap-1 mt-2">';
    if (isChamp) { const c = RECORD_BADGES['Championship']; h += `<span class="badge ${c.bg} ${c.color}" title="DB Trophy Champion"><i class="fa-solid ${c.icon}"></i></span>`; }
    if (dyn?.bonus && dyn.bonus !== '-') dyn.bonus.split(',').map(b => b.trim()).forEach(b => {
        if (b === 'Championship') return;
        const key = Object.keys(RECORD_BADGES).find(k => b.toLowerCase().includes(k.toLowerCase())) ;
        const c = (key && RECORD_BADGES[key]) || { icon: 'fa-medal', color: 'text-slate-400', bg: 'bg-slate-400/10' };
        h += `<span class="badge ${c.bg} ${c.color}" title="${esc(b)}"><i class="fa-solid ${c.icon}"></i></span>`;
    });
    const briggs = currentBriggs();
    if (dyn && briggs && dyn.sheetName === briggs.owner) {
        const c = RECORD_BADGES['The Briggs'];
        h += `<span class="badge ${c.bg} ${c.color}" title="Reigning Briggs holder (${briggs.season}) — the Trophy of Shame"><i class="fa-solid ${c.icon}"></i></span>`;
    }
    return h + '</div>';
}
...
// ─── NAVIGATION ─────────────────────────────────────────────────────────────
const SECTIONS = ['hq', 'league', 'recap', 'dynasty', 'wire', 'analytics', 'lab', 'arcade', 'legacy'];
const VALID_OPTS = { dynasty: ['official', 'audit'], wire: ['activity', 'trending'],
                     legacy: ['records', 'history', 'briggs', 'owners', 'constitution'],
                     recap: ['preview', 'weekly', 'season'] };
const currentOpt = sec => sec === 'dynasty' ? (APP.dynastyTab || 'official')
...
function applyHash(initial) {
    ...
    if (!initial && sec === APP.section && (!opt || opt === currentOpt(sec))) return;
    go(sec, opt, true);
}
...
async function render() {
    ...
    try {
        switch (APP.section) {
            ...
            case 'league':    return await renderLeague(v);
            ...
            case 'arcade':    return renderArcade(v);
...
// ═════════════════════════════════════════════════════════════════════════
//  HQ — THE BARROOM DASHBOARD
// ═════════════════════════════════════════════════════════════════════════
async function renderHQ(v) {
    v.innerHTML = spinner('Pouring the first round…');
    const [ctx, fc] = await Promise.all([API.leagueContext(), API.fcValues().catch(() => null)]);
    const pm = await API.playerMap();

    let recent = [];
    try {
        const list = await API.allTransactions(ctx.id, ctx.inSeason ? ctx.currentWeek : 1);
        recent = list.sort((a, b) => b.created - a.created).slice(0, 8);
    } catch (e) { /* non-fatal */ }
    ...
    // division cards
    const divCards = [1, 2].map(divId => {
        const divTeams = ctx.teams.filter(t => t.division === divId);
        const rows = divTeams.map(team => `
            <div class="flex items-center gap-3 py-1">
                <span class="mono text-[10px] w-4 text-slate-500">${team.standing}</span>
                ${avatarImg(team.avatar, 'w-7 h-7 rounded')}
                ...
                <span class="mono text-[10px] text-slate-600 hidden sm:inline">${Math.round(team.fpts)}</span>
            </div>`).join('');
        return `
        <div class="panel rounded-2xl overflow-hidden cursor-pointer hover:border-white/20 transition-all group" onclick="go('league')">
            <div class="px-5 pt-4 pb-3 border-b border-white/5" style="box-shadow: inset 0 3px 0 ${divId === 1 ? '#F5A623' : '#7FB8D4'}">
                <div class="flex justify-between items-baseline">
                    <h3 class="display text-xl uppercase" style="color:${divId === 1 ? '#F5A623' : '#7FB8D4'}">${esc(DIVISIONS[divId] || 'Division ' + divId)}</h3>
                    <span class="mono text-[9px] tracking-[0.2em] text-slate-600">${divId === 1 ? 'SERVED STRAIGHT' : 'OVER ICE'}</span>
                </div>
            </div>
            <div class="p-5 space-y-1.5">${rows || '<p class="text-sm text-slate-500">Divisions set at draft.</p>'}</div>
        </div>`;
    }).join('');

    const txRows = recent.length ? recent.map(t => txRowHTML(t, ctx, pm)).join('')
        : `<p class="text-sm text-slate-500 p-4">No recent activity. The bar is quiet.</p>`;

    let movers = '';
    if (fc) {
        const rostered = new Set();
        ctx.teams.forEach(team => team.players.forEach(p => rostered.add(p)));
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="panel rounded-xl p-4">
                <p class="mono text-[9px] uppercase tracking-[0.25em] text-emerald-400/70 mb-2"><i class="fa-solid fa-arrow-trend-up mr-1"></i>Rising (30d, rostered)</p>
                ${up.map(e => row(e, true)).join('')}
            </div>
            <div class="panel rounded-xl p-4">
                <p class="mono text-[9px] uppercase tracking-[0.25em] text-red-400/70 mb-2"><i class="fa-solid fa-arrow-trend-down mr-1"></i>Falling (30d, rostered)</p>
    ...
    let seasonLabel;
    if (ctx.inSeason) seasonLabel = `Week ${ctx.currentWeek} · Live`;
    else {
        const start = ctx.state?.season_start_date ? new Date(ctx.state.season_start_date + 'T13:00:00') : null;
        const days = start ? Math.ceil((start - Date.now()) / 864e5) : null;
        seasonLabel = `${SEASON} Offseason · Season ${SEASON_NUM} of the 2nd Dynasty` + (days > 0 ? ` · 🍺 kickoff in ${days} day${days === 1 ? '' : 's'}` : '');
    }

    const briggs = currentBriggs();
    v.innerHTML = `
        ${sectionHead('Last Call HQ', `THE&nbsp;D.B.&nbsp;<span class="grad-gold">LEAGUE</span>`, seasonLabel)}

        <div class="panel-gold rounded-2xl p-5 mb-8">
            <div class="flex flex-col md:flex-row md:items-center gap-4">
                <div class="flex items-center gap-4 flex-1">
                    <img src="logos/league.png" class="w-14 h-14 rounded-xl object-cover border border-[#F5A623]/30">
                    <div>
                        <p class="mono text-[9px] uppercase tracking-[0.3em] text-slate-500">${esc(DYNASTY_LABEL)} · Rankings Leader</p>
                        <p class="display text-2xl uppercase grad-gold">${esc(DYNASTY_STATUS.leader)}</p>
                        <p class="mono text-[9px] text-slate-600 uppercase tracking-widest mt-0.5">1st Dynasty: ${esc(DYNASTY_STATUS.prevVictor)}</p>
                    </div>
                </div>
                <div class="text-right">
                    <span class="mono text-3xl font-bold text-[#F5A623]">${DYNASTY_STATUS.leaderScore.toFixed(2)}</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">${divCards}</div>

        ${briggs ? `
        <div class="panel rounded-2xl p-4 mb-8 flex items-center gap-4 border-l-4 !border-l-amber-700">
            <i class="fa-solid fa-poo text-2xl text-amber-700"></i>
            <div class="flex-1">
                <p class="mono text-[9px] uppercase tracking-[0.3em] text-amber-700/80">Reigning holder of The Briggs</p>
                <p class="text-sm text-slate-300"><b>${esc(briggs.owner)}</b> — “${esc(briggs.team)}” (${briggs.season}). Zero keepers until the poop hat comes out.</p>
            </div>
            <button onclick="go('legacy','briggs')" class="pill">Hall of Shame →</button>
        </div>` : ''}
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div class="panel rounded-2xl p-5">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <button onclick="go('wire')" class="mono text-[9px] uppercase tracking-widest text-[#F5A623]/80 hover:text-[#F5A623]">Full wire →</button>
                </div>
...
// ═════════════════════════════════════════════════════════════════════════
//  LEAGUE — STANDINGS BY DIVISION + LIVE MATCHUPS + TEAM CARDS
// ═════════════════════════════════════════════════════════════════════════
async function renderLeague(v) {
    v.innerHTML = spinner('Setting up the tab…');
    const [ctx, fc] = await Promise.all([API.leagueContext(), API.fcValues().catch(() => null)]);
    ...
    if (ctx.inSeason) {
        ...
        const cards = Object.values(pairs).filter(p => p.length === 2).map(([a, b]) => {
            ...
            return `<div class="panel rounded-xl p-4">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <span class="mono text-lg ${aLead ? 'text-[#F5A623]' : 'text-slate-400'}">${a.points.toFixed(1)}</span>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <span class="mono text-lg ${!aLead ? 'text-[#F5A623]' : 'text-slate-400'}">${b.points.toFixed(1)}</span>
                </div>
            </div>`;
        }).join('');
        ...
        matchupsHTML = `<div class="panel rounded-xl p-4 mb-10 flex items-center gap-3 text-sm text-slate-400">
            <i class="fa-solid fa-hourglass-half text-[#F5A623]/60"></i>
            Offseason — the ${SEASON} season hasn't kicked off. Standings show last synced state; roster values below are live.</div>`;
    }
    ...
    const cards = ctx.teams.map(team => {
        ...
        <div class="panel ${gold ? 'panel-gold' : ''} rounded-2xl p-5 cursor-pointer hover:-translate-y-0.5 hover:border-white/20 transition-all"
             onclick="openTeam(${team.rosterId})">
            <div class="flex justify-between items-start mb-3">
                <div class="flex gap-3 items-center min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mono text-[9px] text-slate-500 uppercase truncate">@${esc(team.display)} · ${esc((DIVISIONS[team.division] || '').replace('Division ', ''))} · since ${dyn?.entered || '—'}</p>
                    </div>
                </div>
                <div class="text-right flex-shrink-0">
                    <span class="display text-2xl ${gold ? 'text-[#F5A623]' : ''}">#${team.standing}</span>
                </div>
            </div>
            ${badgesHTML(dyn, isChamp)}
            <div class="grid grid-cols-5 gap-2 mt-4 pt-3 border-t border-white/5">
                ...
                <div><span class="stat-label">D-Score</span><span class="stat-num ${dyn ? 'text-[#F5A623]' : 'text-slate-500'}">${dyn ? dyn.score.toFixed(0) : '—'}</span></div>
            ...
            ${spark ? `<div class="mt-2">${spark}<span class="stat-label mt-0.5">weekly pts</span></div>` : ''}
        </div>`;
    }).join('');

    v.innerHTML = `
        ${sectionHead('Ten Teams, One Trophy', `THE <span class="grad-gold">LEAGUE</span>`, `${esc(DIVISIONS[1])} · ${esc(DIVISIONS[2])} · 10 teams`)}
        ${matchupsHTML}
...
//  TEAM MODAL
// ═════════════════════════════════════════════════════════════════════════
async function openTeam(rosterId) {
    const modal = document.getElementById('modal');
    const body = document.getElementById('modal-body');
    modal.classList.remove('hidden'); document.body.classList.add('modal-open');
    body.innerHTML = spinner('Checking the tab…');

    const [ctx, fc, pm] = await Promise.all([API.leagueContext(), API.fcValues().catch(() => null), API.playerMap()]);
        ...
        marketHTML = `
            <div class="panel rounded-xl p-4 space-y-3">
                <div class="flex justify-between items-baseline">
                    ...
                    <span class="mono text-[9px] text-slate-500">#${rank} of ${ctx.teams.length}</span>
                </div>
                ...
                <p class="mono text-[8px] text-slate-600 uppercase tracking-widest pt-1">FantasyCalc dynasty SF values · 6h refresh</p>
            </div>`;
    ...
    const dscoreHTML = dyn?.breakdown ? `
        <div class="panel rounded-xl p-4">
            <div class="flex justify-between items-center mb-2">
                ...
                ${dynRank ? `<span class="mono text-[9px] px-2 py-0.5 rounded bg-[#F5A623]/10 text-[#F5A623]">D-#${dynRank}</span>` : ''}
            </div>
            <span class="display text-3xl text-[#F5A623]">${dyn.score.toFixed(2)}</span>
            <div class="mt-3 pt-2 border-t border-white/5 space-y-1">
                ${DSCORE_LINE_ITEMS.filter(li => dyn.breakdown[li.key]).map(li => {
                    ...
                    const neg = ['lastPlace', 'ptsLast', 'b2bLast'].includes(li.key);
    ...
    body.innerHTML = `
        <div class="flex items-start gap-4 mb-6">
            ...
            <div class="flex-1 min-w-0">
                <p class="mono text-[9px] uppercase tracking-[0.3em] text-[#F5A623]">${esc(DIVISIONS[team.division] || '')} · #${team.standing}</p>
                <h2 class="display text-2xl sm:text-3xl uppercase leading-tight">${esc(team.name)}</h2>
                <p class="mono text-[10px] text-slate-500">@${esc(team.display)} · since ${dyn?.entered || '—'} · ${team.wins}-${team.losses} · ${Math.round(team.fpts)} PF</p>
                ${badgesHTML(dyn, dyn && CHAMPION_TEAMS.includes(dyn.team))}
            </div>
        </div>
...
// ═════════════════════════════════════════════════════════════════════════
//  DYNASTY — OFFICIAL RANKINGS + AUTO-AUDIT
// ═════════════════════════════════════════════════════════════════════════
async function renderDynasty(v) {
    ...
    v.innerHTML = spinner('Computing who buys the next round…');
    let fc = null, ctx = null;
    try { [fc, ctx] = await Promise.all([API.fcValues(), API.leagueContext()]); } catch (e) { /* manual table still renders */ }

    const mvByTeam = {};
    if (fc && ctx) ctx.teams.forEach(team => {
        const dyn = getDynastyInfo(team.user, team.name);
        if (dyn) mvByTeam[dyn.team] = rosterValue(team.players, fc).total;
    });

    const rows = dynastySorted().map((item, i) => {
        const isChamp = CHAMPION_TEAMS.includes(item.team);
        const scoreCls = i === 0 ? 'text-[#F5A623]' : 'text-white';
        const mv = mvByTeam[item.team];
        return `<tr class="border-b border-white/5 hover:bg-white/5 transition-colors ${i === 0 ? 'bg-[#F5A623]/5' : ''}">
            <td class="p-3 sm:p-4 display text-xl ${i === 0 ? 'text-[#F5A623]' : 'text-slate-400'}">${i + 1}</td>
            <td class="p-3 sm:p-4">
                <p class="font-bold uppercase tracking-tight text-sm ${i === 0 ? 'grad-gold' : ''}">${esc(item.team)}</p>
                <p class="mono text-[9px] text-slate-600">${esc(item.sheetName)} · since ${item.entered || '—'}</p>
            </td>
            ...
            <td class="p-3 sm:p-4 mono text-xs text-sky-300 hidden sm:table-cell">${mv ? fmtVal(mv) : '—'}</td>
            <td class="p-3 sm:p-4 hidden md:table-cell">${badgesHTML(item, isChamp)}</td>
        </tr>`;
    }).join('');

    v.innerHTML = `
        ${sectionHead('The Standings of History', 'DYNASTY <span class="grad-gold">RANKINGS</span>',
            'Amendment I scoring — the ledger of who has actually been good at this. Market value is each roster’s live FantasyCalc valuation: past glory vs. present arsenal.')}
        ${subTabs}
        ${APP.sheetLive
...
async function renderDScoreAudit(v, subTabs) {
    v.innerHTML = spinner('Auditing the bar tab…');
    const hist = await API.leagueHistory();
    ...
    const rows = dynastySorted().map(item => {
        ...
        return `<tr class="border-b border-white/5 hover:bg-white/5 cursor-pointer" onclick="document.getElementById('aud-${rid}').classList.toggle('hidden')">
            <td class="p-3"><p class="text-sm font-bold uppercase">${esc(item.team)}</p>
                <p class="mono text-[9px] text-slate-600">avg win% ${c.avgWinPct.toFixed(1)} · manual bonuses ${c.manualBonus}</p></td>
    ...
    v.innerHTML = `
        ${sectionHead('The Commissioner’s Audit', 'D-SCORE <span class="grad-gold">AUTO-AUDIT</span>',
            'Every Amendment I component computed live from Sleeper history — championships from brackets, The Briggs from the toilet bowl, points titles from scoring. Yellow components disagree with the official books. Sleeper era only; the ESPN years live in the manual bonuses.')}
        ${subTabs}
        ...
        <p class="mono text-[9px] text-slate-600 uppercase tracking-widest mt-3">Computed = components × Amendment I points + avg win% + manual bonuses · click a row for the component comparison</p>`;
}

// ═════════════════════════════════════════════════════════════════════════
//  THE WIRE — TRANSACTIONS + TRENDING
// ═════════════════════════════════════════════════════════════════════════
function txRowHTML(t, ctx, pm) {
    ...
    if (t.type === 'trade') {
        ...
        return `<div class="py-2.5 text-xs leading-relaxed">
            <span class="tx-tag bg-purple-400/10 text-purple-300">TRADE</span>
    ...
    return `<div class="py-2.5 text-xs leading-relaxed">
        <span class="tx-tag ${isWaiver ? 'bg-sky-400/10 text-sky-300' : 'bg-emerald-400/10 text-emerald-300'}">${isWaiver ? 'WAIVER' : 'FA'}</span>
        <span class="mono text-[9px] text-slate-600 ml-1">wk${t.leg} · ${fmtDate(t.created)}</span>
...
async function renderWire(v) {
    v.innerHTML = spinner('Tapping the phones…');
    ...
    if (APP.wireTab === 'trending') {
        const [ctx, pm, adds, drops, fc] = await Promise.all([
            API.leagueContext(), API.playerMap(),
            API.trending('add'), API.trending('drop'), API.fcValues().catch(() => null)]);
        const rostered = new Set();
        ctx.teams.forEach(team => team.players.forEach(p => rostered.add(p)));
        const row = (e, isAdd) => {
            const p = pm[e.player_id]; if (!p) return '';
            const avail = !rostered.has(e.player_id);
            ...
            return `<div class="flex items-center gap-3 py-2.5 border-b border-white/5 last:border-0">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="mono text-[9px] ${avail ? 'text-emerald-400/80' : 'text-slate-600'}">${avail ? 'available in the DB League' : 'already rostered'}</p>
                </div>
        ...
        v.innerHTML = `
            ${sectionHead('The Wire', 'TRENDING <span class="grad-gold">PLAYERS</span>',
                'Hottest adds and drops across all of Sleeper (last 48h), cross-checked against DB rosters — green means somebody here can still grab them.')}
            ${tabs}
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
    ...
    const [ctx, pm] = await Promise.all([API.leagueContext(), API.playerMap()]);
    const all = (await API.allTransactions(ctx.id, ctx.inSeason ? ctx.currentWeek : 1))
        .sort((a, b) => b.created - a.created);

    window._wireData = { all, pm };
    ...
    function buildShell(listHTML, counts) {
        ...
        v.innerHTML = `
            ${sectionHead('The Wire', 'LEAGUE <span class="grad-gold">ACTIVITY</span>',
                'Every trade, waiver claim, and free-agent move — pulled live from Sleeper. No more he-said-she-said.')}
            ${tabs}
    ...
    function renderWireList(typeF) {
        window._wireFilter = typeF;
        const filtered = all.filter(t => typeF === 'all' || t.type === typeF).slice(0, 150);
        const listHTML = filtered.length ? filtered.map(t => txRowHTML(t, ctx, pm)).join('')
        ...
        if (!shellBuilt || !document.getElementById('wire-list')) buildShell(listHTML, counts);
    ...
    renderWireList('all');
    window.filterWire = key => renderWireList(key);
}

// ═════════════════════════════════════════════════════════════════════════
//  ANALYTICS — POWER · PLAYOFF ODDS · BRIGGS ODDS
// ═════════════════════════════════════════════════════════════════════════
    ...
    if (ctx.inSeason && ctx.completedWeeks >= 2) {
        ...
        const rows = power.map((p, i) => {
            ...
            return `<tr class="border-b border-white/5 hover:bg-white/5">
                ...
                <td class="p-3">${sparkline(p.team.weeklyScores, 80, 22)}</td>
                <td class="p-3 mono text-xs ${o.playoff > 0.5 ? 'text-emerald-400' : 'text-slate-400'}">${o.playoff !== undefined ? (o.playoff * 100).toFixed(0) + '%' : '—'}</td>
                <td class="p-3 mono text-xs ${o.briggs > 0.25 ? 'text-amber-600' : 'text-slate-500'}">${o.briggs !== undefined ? (o.briggs * 100).toFixed(0) + '%' : '—'}</td>
            </tr>`;
        }).join('');
        ...
        <div class="panel rounded-2xl overflow-hidden">
            ...
            <table class="w-full text-left">
                <thead><tr class="bg-white/5 mono text-[9px] uppercase tracking-[0.2em] text-slate-400">
                    ...
                    <th class="p-3">Form (3wk)</th><th class="p-3">Trend</th><th class="p-3">Playoff%</th><th class="p-3" title="Odds of finishing last in the sim — poop hat probability">Briggs%</th>
                </tr></thead><tbody>${rows}</tbody>
            </table></div>
        </div>
        <p class="mono text-[9px] text-slate-600 uppercase tracking-widest mt-3">Power = 70% all-play + 30% recent form · Odds = 2,000-season Monte Carlo on remaining schedule · Briggs% = last-by-record proxy for the toilet bowl</p>`;
    } else {
        if (!fc) { content = errBox('Market data unavailable for preseason outlook.'); }
            ...
            const rows = rated.map((r, i) => {
                const status = r.total > median * 1.15 ? ['CONTENDER', 'text-emerald-400'] :
                               r.total < median * 0.85 ? ['BRIGGS BAIT', 'text-amber-600'] : ['IN THE HUNT', 'text-slate-300'];
                return `<div class="panel rounded-xl p-4 flex items-center gap-4">
                    <span class="display text-2xl w-8 ${i === 0 ? 'text-[#F5A623]' : 'text-slate-500'}">${i + 1}</span>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="h-1 bg-white/5 rounded-full overflow-hidden mt-1.5">
                            <div class="h-full rounded-full bg-[#F5A623]" style="width:${Math.round(r.total / maxV * 100)}%"></div>
                        </div>
                    </div>
                    <div class="text-right flex-shrink-0">
            ...
            content = `
                <div class="panel rounded-xl p-4 mb-5 text-sm text-slate-400 flex items-center gap-3">
                    <i class="fa-solid fa-binoculars text-[#F5A623]/70"></i>
                    Preseason outlook — power rankings, all-play, playoff odds and Briggs odds activate once Week 1 kicks off. Until then: the market's view of every roster.</div>
                <div class="grid grid-cols-1 gap-3">${rows}</div>`;
        }
    }

    v.innerHTML = `
        ${sectionHead('The Numbers', 'ANALYTICS',
...
async function monteCarloOdds(ctx) {
    ...
    try {
        ...
        const tally = {}; ctx.teams.forEach(t => tally[t.rosterId] = { playoff: 0, briggs: 0 });
        for (let s = 0; s < SIMS; s++) {
            ...
            tally[order[order.length - 1]].briggs++;
        }
        ctx.teams.forEach(t => out[t.rosterId] = {
            playoff: tally[t.rosterId].playoff / SIMS, briggs: tally[t.rosterId].briggs / SIMS });
...
async function renderLab(v) {
    v.innerHTML = spinner('Opening the back room…');
    const [ctx, fc, pm] = await Promise.all([API.leagueContext(), API.fcValues(), API.playerMap()]);
    ...
    const sideHTML = (side) => {
        ...
        const checks = valued.map(({ pid, e }) => `
            <label class="flex items-center gap-2.5 py-1.5 px-2 rounded hover:bg-white/5 cursor-pointer ${selSet.has(String(pid)) ? 'bg-[#F5A623]/10' : ''}">
                <input type="checkbox" ${selSet.has(String(pid)) ? 'checked' : ''} onchange="labToggle('${side}','${pid}')" class="accent-[#F5A623]">
    ...
    if (tot > 0) {
        ...
        verdict = skew < 0.07 ? '⚖️ Dead even — even Mark couldn’t complain.'
            : skew < 0.2 ? `Slight edge to ${diff > 0 ? 'Side B (receiving A’s package)' : 'Side A (receiving B’s package)'}.`
            : `🚨 Lopsided — ${diff > 0 ? 'Side A is giving up' : 'Side B is giving up'} ${fmtVal(Math.abs(diff))} more in value. Legal, but shameable (Art. VII). If a Baby-Owner is involved, the commissioner is watching (Amendment XII).`;
    }

    v.innerHTML = `
        ${sectionHead('The Back Room', 'TRADE <span class="grad-gold">LAB</span>',
            'Weigh any deal with live FantasyCalc superflex values. Picks priced at market. Reminder: Baby-Owner trades must favor the kid by 25% or the commissioner intervenes.')}
        <div class="panel-gold rounded-2xl p-5 mb-6">
            ...
            <div class="h-2.5 rounded-full overflow-hidden bg-white/5 flex">
                <div class="h-full bg-gradient-to-r from-[#F5A623] to-[#c47d0e] transition-all" style="width:${pctA}%"></div>
...
// ═════════════════════════════════════════════════════════════════════════
//  ARCADE — Endzone Run launcher (game lives at arcade/, own PWA)
// ═════════════════════════════════════════════════════════════════════════
function renderArcade(v) {
    v.innerHTML = `
        ${sectionHead('The Back Alley', 'ENDZONE <span class="grad-gold">RUN</span>',
            'Flick your runner through all 10 DB defenses — four downs to beat every team on the dynasty ladder. Fair warning about the top.')}
        <div class="panel-gold rounded-2xl p-10 sm:p-14 text-center">
            <i class="fa-solid fa-gamepad text-5xl text-[#F5A623]/70 mb-5"></i>
            <p class="display text-3xl sm:text-4xl uppercase mb-3">Endzone <span class="grad-gold">Run</span></p>
            <p class="text-sm text-slate-400 max-w-md mx-auto leading-relaxed mb-6">Madden-style kick meter. Every defense wears
            its real DB team colors — bunnies included. Earn yards, buy upgrades, climb the ladder from the Wankers
            to the Dirty Dawgs. Best on desktop or a landscape phone — installs to your home screen like an app.</p>
...
// ═════════════════════════════════════════════════════════════════════════
//  LEGACY — RECORDS · HISTORY · THE BRIGGS · OWNERS · CONSTITUTION
// ═════════════════════════════════════════════════════════════════════════
async function loadRecordBook() {
    ...
    try {
        const hist = await API.leagueHistory();
        const liveCtx = hist.some(s => !s.complete) ? await API.leagueContext() : null;
        ...
        if (!games.length) { box().innerHTML = errBox('No game history found yet.'); return; }
        ...
        const lows = [...games].sort((a, b) => a.pts - b.pts);
        ...
        const gameRows = list => list.slice(0, 10).map((g, i) => `
            <div class="flex items-center gap-3 py-1.5 border-b border-white/5 last:border-0 text-sm">
                <span class="mono text-[10px] w-5 ${i === 0 ? 'text-[#F5A623]' : 'text-slate-500'}">${i + 1}</span>
                ...
                <span class="mono text-sm ${i === 0 ? 'text-[#F5A623] font-bold' : 'text-slate-300'}">${g.pts.toFixed(2)}</span>
            </div>`).join('');
        const miniRows = (list, fmt) => list.slice(0, 5).map((e, i) => `
            <div class="flex items-center gap-3 py-1.5 border-b border-white/5 last:border-0 text-sm">
                ...
                <span class="mono text-sm ${i === 0 ? 'text-[#F5A623] font-bold' : 'text-slate-300'}">${fmt(e)}</span>
            </div>`).join('');
        ...
        const threatBox = threats.length ? `
            <div class="panel-gold rounded-2xl p-4 mb-5 text-sm text-slate-300">
                <p class="mono text-[9px] uppercase tracking-[0.3em] text-[#F5A623]/80 mb-2"><i class="fa-solid fa-triangle-exclamation mr-1.5"></i>Record Watch — history under threat</p>
                ${threats.map(t => `<p class="py-0.5">· ${esc(t)}</p>`).join('')}
            </div>` : '';

        box().innerHTML = `
            <h3 class="display text-xl uppercase mb-1 text-slate-300">The Record Book <span class="mono text-[9px] text-slate-600 not-italic tracking-widest ml-2">AUTO · SLEEPER ERA</span></h3>
            <p class="mono text-[9px] uppercase tracking-[0.25em] text-slate-600 mb-4">Computed live from every box score · ● = current season, still moving</p>
            ${threatBox}
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ${listCard('Top 10 Single-Game Scores', 'fa-fire', 'text-orange-400', gameRows(games))}
                <div class="space-y-5">
                    ${listCard('Lowest Single-Game Scores', 'fa-toilet', 'text-slate-400', miniRows(lows, e => e.pts.toFixed(2)))}
...
function canonicalOwnerId(id) { return id; }
async function buildOwnerCareers() {
    const hist = await API.leagueHistory();
    const owners = {};
    const activeIds = new Set(DYNASTY_RANKS.map(d => d.ownerId));
    hist.forEach(s => s.teams.forEach(t => {
        if (!t.ownerId || !activeIds.has(t.ownerId)) return; // active 10 only — departed owners live in History
        const o = owners[t.ownerId] ||= { id: t.ownerId, displays: new Set(), avatar: null, w: 0, l: 0, t: 0, pf: 0, seasons: [] };
        o.displays.add(t.display);
        if (!o.avatar && t.avatar) o.avatar = t.avatar;
        o.w += t.wins; o.l += t.losses; o.t += t.ties; o.pf += t.fpts;
        o.seasons.push({ season: s.season, teamName: t.teamName, wins: t.wins, losses: t.losses,
                         fpts: t.fpts, standing: t.standing, of: s.teams.length, live: !s.complete });
    }));
    Object.values(owners).forEach(o => {
        o.seasons.sort((a, b) => b.season - a.season);
        const g = o.w + o.l + o.t;
        o.winPct = g ? o.w / g : 0;
        const dyn = DYNASTY_RANKS.find(d => d.ownerId === o.id);
        o.dyn = dyn || null;
        o.titles = dyn ? HALL_OF_FAME.filter(h => h.owner.toLowerCase().startsWith(dyn.sheetName.toLowerCase())).length : 0;
    });
...
async function renderOwnersTab(v, tabs) {
    v.innerHTML = spinner('Pulling everyone’s tab history…');
    ...
    const rows = _careers.map((o, i) => `
        <tr class="border-b border-white/5 hover:bg-white/5 cursor-pointer" onclick="openOwner('${o.id}')">
            ...
            <td class="p-3"><div class="flex items-center gap-2.5">
                ...
                <div class="min-w-0">
                    <p class="text-sm font-bold truncate">${o.dyn ? esc(o.dyn.sheetName) : '@' + esc([...o.displays][0])}</p>
            ...
            <td class="p-3">${o.titles ? `<span class="badge bg-yellow-400/10 text-yellow-400" title="${o.titles} DB Trophy(s), 2nd Dynasty"><i class="fa-solid fa-trophy"></i></span>` : ''}
                ${o.dyn ? `<span class="mono text-[9px] text-[#F5A623] ml-1">${o.dyn.score.toFixed(0)}</span>` : ''}</td>
        </tr>`).join('');
    v.innerHTML = `${sectionHead('Every Buddy, Every Season', 'OWNER <span class="grad-gold">CAREERS</span>',
        'The active ten, career records rebuilt season-by-season from Sleeper history (2nd Dynasty era). Departed owners live in the History tab; the ESPN years live in the 1st Dynasty archive. Click an owner for their full log.')}${tabs}
        <div class="panel rounded-2xl overflow-hidden"><div class="overflow-x-auto">
        <table class="w-full text-left">
            <thead><tr class="bg-white/5 mono text-[9px] uppercase tracking-[0.2em] text-slate-400">
                <th class="p-3">#</th><th class="p-3">Owner</th><th class="p-3">W-L</th><th class="p-3">Win%</th>
                <th class="p-3 hidden sm:table-cell">PF</th><th class="p-3">Honors · D-Score</th>
            </tr></thead><tbody>${rows}</tbody>
        </table></div></div>`;
}
...
window.openOwner = id => {
    ...
    const log = o.seasons.map(s => `
        <tr class="border-b border-white/5">
            ...
            <td class="p-2.5 text-sm truncate max-w-[12rem]">${esc(s.teamName)}</td>
            <td class="p-2.5 mono text-xs">${s.wins}-${s.losses}</td>
            <td class="p-2.5 mono text-xs text-slate-400">${Math.round(s.fpts)}</td>
            <td class="p-2.5 mono text-xs ${s.standing === 1 ? 'text-[#F5A623]' : s.standing <= 3 ? 'text-emerald-400' : 'text-slate-400'}">${s.standing}/${s.of}</td>
        </tr>`).join('');
    document.getElementById('modal-body').innerHTML = `
        <div class="flex items-center gap-4 mb-5">
            ${avatarImg(o.avatar, 'w-16 h-16 rounded-xl')}
            <div>
                <h2 class="display text-2xl uppercase">${o.dyn ? esc(o.dyn.sheetName) : '@' + esc([...o.displays][0])}</h2>
                <p class="mono text-[10px] text-slate-500">${o.dyn ? esc(o.dyn.team) + ' · ' : ''}${o.w}-${o.l} · ${(o.winPct * 100).toFixed(0)}% · ${fmtVal(Math.round(o.pf))} PF
                ${o.titles ? ` · <span class="text-yellow-400">${o.titles}× DB Trophy</span>` : ''}</p>
                ${o.dyn ? badgesHTML(o.dyn, CHAMPION_TEAMS.includes(o.dyn.team)) : ''}
            </div>
        </div>
        <div class="panel rounded-xl overflow-hidden"><div class="overflow-x-auto">
        <table class="w-full text-left">
            <thead><tr class="bg-white/5 mono text-[9px] uppercase tracking-[0.2em] text-slate-400">
                <th class="p-2.5">Year</th><th class="p-2.5">Team</th>
...
async function renderLegacy(v) {
    ...
    const labels = { records: 'Records', history: 'History', briggs: 'The Briggs', owners: 'Owners', constitution: 'Constitution' };
    const tabs = `<div class="flex gap-2 mb-8 flex-wrap">
        ${Object.entries(labels).map(([t, l]) => `
        <button onclick="go('legacy','${t}')" class="pill ${tab === t ? 'pill-active' : ''}">${l}</button>`).join('')}</div>`;

    if (tab === 'owners') return renderOwnersTab(v, tabs);

    if (tab === 'briggs') {
        const briggs = currentBriggs();
        const rows = BRIGGS_WINNERS.map((b, i) => `
            <div class="panel rounded-2xl p-5 ${i === 0 ? '!border-amber-700/50' : ''} relative overflow-hidden">
                <i class="fa-solid fa-poo absolute -right-3 -bottom-3 text-6xl opacity-5"></i>
                <p class="mono text-[9px] uppercase tracking-[0.25em] ${i === 0 ? 'text-amber-600' : 'text-slate-500'} mb-1">
                    ${b.season}${i === 0 ? ' · REIGNING' : ''}</p>
                <p class="display text-xl uppercase">${esc(b.owner)}</p>
                <p class="mono text-[10px] text-slate-500 mt-1">“${esc(b.team)}”</p>
            </div>`).join('');
        v.innerHTML = `${sectionHead('The Trophy of Shame', 'THE <span class="text-amber-600">BRIGGS</span>',
            'Named for Christopher “Briggs” Briggs, original winner (2017). Awarded to last place, who starts the next season with ZERO keepers — redeemable in thirds via poop-emoji-hat photo, trophy-topper funding, and public humiliation at a rival stadium. The trophy itself lives at Dan’s bar, mocking everyone on this list. We only mock the ones we love.')}${tabs}
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">${rows}</div>
            <p class="mono text-[9px] text-slate-600 uppercase tracking-widest mt-4">Several years had losers deemed unworthy of even The Briggs.</p>`;
        return;
    }
    ...
    if (tab === 'records') {
        ...
        const recs = DYNASTY_RECORDS.map(r => `
            <div class="panel rounded-2xl p-5 relative overflow-hidden">
                ...
                <p class="mono text-[10px] text-[#F5A623]/80 uppercase mt-1">${esc(r.owner)}</p>
            </div>`).join('');
        v.innerHTML = `${sectionHead('Etched in the Bar Top', 'RECORDS & <span class="grad-gold">HONORS</span>')}${tabs}
            <h3 class="display text-xl uppercase mb-4 text-slate-300">DB Trophy Winners · 2nd Dynasty</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5 mb-10">${hof}</div>
            <h3 class="display text-xl uppercase mb-4 text-slate-300">Permanent Records · 2nd Dynasty</h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">${recs}</div>
    ...
    if (tab === 'history') {
        const nodes = SEASON_HISTORY.map((s, i) => `
            <div class="relative pl-8 pb-10 ${i === SEASON_HISTORY.length - 1 ? '' : 'timeline-seg'}">
                <div class="absolute left-0 top-1 w-3.5 h-3.5 rounded-full border-2 ${s.live ? 'border-red-400 bg-red-400/20 animate-pulse' : 'border-[#F5A623] bg-[#14100B]'}"></div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="panel rounded-xl p-4 space-y-1.5 text-sm text-slate-400">
                        ...
                        ${s.facts.map(f => `<p><i class="fa-solid fa-angle-right text-[#F5A623]/50 text-xs mr-2"></i>${esc(f)}</p>`).join('')}
                    </div>
        ...
        const fd = FIRST_DYNASTY;
        const fdStandings = fd.finalStandings.map(([n, s], i) => `
            <div class="flex justify-between py-1 border-b border-white/5 last:border-0 text-sm">
                <span>${i + 1}. ${esc(n)}</span><span class="mono ${s < 0 ? 'text-red-400' : 'text-slate-300'}">${s.toFixed(2)}</span>
            </div>`).join('');
        const fdChamps = fd.champions.map(c => `
            <div class="flex justify-between py-1 border-b border-white/5 last:border-0 text-sm">
                <span class="mono text-xs text-slate-500 w-12">${c.season}</span>
                <span class="flex-1 truncate px-2">${esc(c.team)}</span>
                <span class="text-slate-400">${esc(c.owner)}</span>
            </div>`).join('');
        const fdRecords = fd.records.map(r => `
            <div class="flex justify-between py-1 border-b border-white/5 last:border-0 text-sm">
                <span class="text-slate-400">${esc(r.label)}</span>
                <span class="mono text-xs">${esc(r.val)} <span class="text-slate-500">· ${esc(r.owner)}</span></span>
            </div>`).join('');
        const retired = RETIRED_OWNERS.map(r => `
            <div class="panel rounded-xl p-4 border-l-2 ${r.honorable ? 'border-l-emerald-700' : 'border-l-red-900'}">
                <p class="mono text-[9px] uppercase tracking-[0.2em] ${r.honorable ? 'text-emerald-500' : 'text-red-500'} mb-1">${r.honorable ? 'Honorable Discharge' : r.detail.includes('BANNED') ? 'Banned' : 'Other than Honorable'}</p>
                <p class="font-bold text-sm">${esc(r.name)}</p>
                <p class="mono text-[10px] text-slate-500">${esc(r.detail)}</p>
            </div>`).join('');
        v.innerHTML = `${sectionHead('Since 2011', 'LEAGUE <span class="grad-gold">HISTORY</span>', '2nd Dynasty · 2022–Present')}${tabs}
            <div class="max-w-3xl">${nodes}</div>
            <h3 class="display text-xl uppercase mt-8 mb-1 text-slate-300">The Frank Dynasty <span class="mono text-[9px] text-slate-600 not-italic tracking-widest ml-2">1ST DYNASTY · 2014–2021</span></h3>
            <p class="mono text-[9px] uppercase tracking-[0.25em] text-slate-600 mb-4">Victor: ${esc(fd.victor)} · absorbing the 2011–2013 prehistory</p>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div class="panel rounded-xl p-4"><p class="stat-label mb-2">Final Standings</p>${fdStandings}</div>
                <div class="panel rounded-xl p-4"><p class="stat-label mb-2">Champions</p>${fdChamps}</div>
                <div class="panel rounded-xl p-4"><p class="stat-label mb-2">1st Dynasty Records</p>${fdRecords}</div>
            </div>
            <h3 class="display text-xl uppercase mt-6 mb-1 text-slate-400">Retired & Banned Owners</h3>
        ...
        return `<div class="const-item panel rounded-xl overflow-hidden" data-text="${esc((item.title + ' ' + item.content).toLowerCase())}" data-id="${item.id}">
            <button onclick="toggleArticle('${item.id}')" class="w-full text-left p-4 flex items-center justify-between gap-4 hover:bg-white/5 transition-colors">
                <span class="flex items-center gap-3">
                    <span class="w-1.5 h-1.5 rounded-full ${isA ? 'bg-[#F5A623]/60' : 'bg-white/30'}"></span>
                    <span class="display text-sm uppercase ${isA ? 'text-[#F5A623]/90' : ''}">${esc(item.title)}</span>
                </span>
    ...
    v.innerHTML = `${sectionHead('House Rules', 'THE <span class="grad-gold">CONSTITUTION</span>',
            `Revision ${CONSTITUTION_META.revision} · Commissioner: ${CONSTITUTION_META.commissioner} · ${CONSTITUTION_META.sidepiece}`)}${tabs}
        <div class="flex flex-col sm:flex-row gap-2 mb-5">
            <input oninput="filterConst(this.value)" type="text" placeholder="Search the house rules…" class="input-dark flex-1">
