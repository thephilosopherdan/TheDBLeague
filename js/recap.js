// ═════════════════════════════════════════════════════════════════════════
//  THE D.B. LEAGUE — RECAP ENGINE
//  Preseason Preview · Weekly Recap · Season Wraps
// ═════════════════════════════════════════════════════════════════════════
...
function computeWeeklyRecap(ctx, wk) {
    ...
    const upset = games.filter(g => (g.wt?.standing || 0) - (g.lt?.standing || 0) >= 3)
...
function weeklyRecapHTML(ctx, wk) {
    const r = computeWeeklyRecap(ctx, wk);
    if (!r) return errBox('No completed games for that week yet.');
    const cards = [
        awardCard('fa-beer-mug-empty', 'text-[#F5A623]', 'Buying the Round', r.high.t?.name || '?', `${r.high.m.points.toFixed(2)} pts — highest at the bar`),
        awardCard('fa-toilet', 'text-amber-700', 'Briggs Watch', r.low.t?.name || '?', `${r.low.m.points.toFixed(2)} pts — the hat awaits`),
        ...
        awardCard('fa-hammer', 'text-orange-400', 'Last Call', `${r.blowout.wt?.name || '?'}`, `closed out ${esc(r.blowout.lt?.name || '?')} by ${r.blowout.margin.toFixed(2)}`)
    ];
    ...
    const board = r.games.map(g => `
        <div class="flex items-center gap-3 py-2 border-b border-white/5 last:border-0 text-sm">
            ...
            <span class="mono text-[#F5A623]">${g.w.points.toFixed(1)}</span>
    ...
    return `<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">${cards.join('')}</div>
        <div class="panel rounded-2xl p-5">
            <h3 class="display text-lg uppercase mb-2 text-[#F5A623]">Week ${wk} Final Scores</h3>${board}
        </div>`;
}
...
function computeSeasonRecap(seasonObj, weekly) {
    ...
    const lfinal = seasonObj.loserBracket?.find(m => m.p === 1);
    return { high, low, closest, blowout,
        ...
        briggs: lfinal && typeof lfinal.l === 'number' ? teamOf(lfinal.l) : seasonObj.teams[seasonObj.teams.length - 1] };
}

function seasonRecapHTML(seasonObj, weekly) {
    const r = computeSeasonRecap(seasonObj, weekly);
    const champBanner = r.champ ? `
        <div class="champ-banner rounded-2xl p-6 mb-6">
            <p class="mono text-[9px] uppercase tracking-[0.3em] text-yellow-200/60 mb-1">${seasonObj.season} DB Trophy Champion</p>
    ...
    const cards = [];
    if (r.bestRecord) cards.push(awardCard('fa-ranking-star', 'text-[#F5A623]', 'Best Record', r.bestRecord.teamName, `${r.bestRecord.wins}-${r.bestRecord.losses} regular season`));
    ...
    if (r.briggs) cards.push(awardCard('fa-poo', 'text-amber-700', 'The Briggs', r.briggs.teamName || '?', 'last place — the hat, the topper, the shame'));
    ...
    if (r.low) cards.push(awardCard('fa-toilet', 'text-slate-400', 'Rock Bottom', r.low.t?.teamName || '?', `${r.low.pts.toFixed(2)} pts · week ${r.low.wk}`));

    const table = seasonObj.teams.map(t => `
        <tr class="border-b border-white/5 ${t.standing === 1 ? 'bg-[#F5A623]/5' : ''}">
            <td class="p-2.5 mono text-xs ${t.standing <= 3 ? 'text-[#F5A623]' : 'text-slate-500'}">${t.standing}</td>
...
function previewHTML(ctx, fc) {
    ...
    const story = `<div class="panel-gold rounded-2xl p-5 mb-6 space-y-2 text-sm text-slate-300">
        <p class="mono text-[9px] uppercase tracking-[0.3em] text-[#F5A623]/70 mb-2">Season ${SEASON_NUM} Storylines</p>
        <p><i class="fa-solid fa-trophy text-yellow-400 text-xs mr-2"></i><strong>${esc(champ.team)}</strong> defends the ${champ.season} DB Trophy — a win would make it a THREE-PEAT, and the trophy might just get legally renamed.</p>
        ${briggs ? `<p><i class="fa-solid fa-poo text-amber-700 text-xs mr-2"></i><strong>${esc(briggs.owner)}</strong> opens the season with zero keepers until the poop-hat obligations are met. The Briggs demands its due.</p>` : ''}
        <p><i class="fa-solid fa-baby text-sky-300 text-xs mr-2"></i>The Baby-Owner era continues — trade with Theo at your own risk (Amendment XII is watching).</p>
        <p><i class="fa-solid fa-triangle-exclamation text-red-400 text-xs mr-2"></i>Redraft trigger check: one more Dan title puts ">½ champs" in play. The league's fate may hinge on stopping him.</p>
    </div>`;
    ...
    const supers = [
        awardCard('fa-sack-dollar', 'text-[#F5A623]', 'Deepest War Chest', rated[0].team.name, `${fmtVal(rated[0].total)} total value · ${(rated[0].total / sumVal * 100).toFixed(0)}% market share`),
        ...
        return `<div class="panel rounded-xl p-4 flex items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="h-1 bg-white/5 rounded-full overflow-hidden mt-1.5">
                    <div class="h-full rounded-full bg-[#F5A623]" style="width:${Math.round(r.total / rated[0].total * 100)}%"></div>
                </div>
            </div>
    ...
    return `${story}
        ...
        <p class="mono text-[9px] text-slate-600 uppercase tracking-widest mt-3">Market share = team value ÷ league value · FantasyCalc dynasty SF 0.5 PPR · refreshed 6h</p>`;
}

// ── SHARE ───────────────────────────────────────────────────────────────────
window.copyWeeklyRecap = async () => {
    const ctx = await API.leagueContext();
    ...
    const L = [`🍺 THE D.B. LEAGUE — Week ${wk} Recap`, ''];
    L.push(`🏆 Buying the Round: ${r.high.t?.name} — ${r.high.m.points.toFixed(2)} pts`);
    L.push(`💩 Briggs Watch: ${r.low.t?.name} — ${r.low.m.points.toFixed(2)} pts`);
    L.push(`📸 Photo Finish: ${r.closest.wt?.name} edged ${r.closest.lt?.name} by ${r.closest.margin.toFixed(2)}`);
    L.push(`🔨 Last Call: ${r.blowout.wt?.name} closed out ${r.blowout.lt?.name} by ${r.blowout.margin.toFixed(2)}`);
    if (r.upset) L.push(`⚡ Upset: #${r.upset.wt.standing} ${r.upset.wt.name} toppled #${r.upset.lt.standing} ${r.upset.lt.name}`);
...
async function renderRecap(v) {
    ...
    if (sub === 'weekly') {
        ...
        v.innerHTML = `${sectionHead('The Press Box', `WEEK ${wk} <span class="grad-gold">RECAP</span>`,
            'Everything that happened at the bar this week')}${subTabs}${wkSel}
            ${weeklyRecapHTML(ctx, wk)}`;
        return;
    }
    ...
    if (sub === 'season') {
        ...
        if (!done.length) {
            v.innerHTML = `${sectionHead('The Archive', 'SEASON <span class="grad-gold">WRAPS</span>')}${subTabs}
                ${errBox('No completed seasons in the Sleeper era yet.')}`;
            return;
        }
        ...
        v.innerHTML = `${sectionHead('The Archive', `${yr} <span class="grad-gold">SEASON WRAP</span>`,
            'The full story of the season')}${subTabs}${yrSel}${spinner('Pulling the season film…')}`;
        const weekly = await API.seasonMatchups(seasonObj.leagueId);
        v.innerHTML = `${sectionHead('The Archive', `${yr} <span class="grad-gold">SEASON WRAP</span>`,
            'The full story of the season')}${subTabs}${yrSel}
            ${seasonRecapHTML(seasonObj, weekly)}`;
        return;
    }
    ...
    v.innerHTML = `${sectionHead('The Press Box', `${SEASON} <span class="grad-gold">PRESEASON PREVIEW</span>`,
        `Season ${SEASON_NUM} of the 2nd Dynasty · the market's read on every roster before a snap is played`)}${subTabs}
        ${previewHTML(ctx, fc)}`;
}
